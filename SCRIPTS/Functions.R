...
findSubs = function(alStr, pks, pos){
	...
	if(length(table(alStrDF3)) == 1){
		return(NULL)
	} else if(sum(names(table(alStrDF3)) == '2') == 0){
		return(NULL)
	} else{
...
ancestralize = function(pkAln, tmpdir = '/home2/s422159/workdir/TMP/', submodel = 'F81', extant_species = 'hg38', phyltree, ancestral_nodes){

	# ancetral_nodes: Needs to be in this order: hominin, african g. ape, g. ape, ape, root node
	...
	fit = pml(phyltree, primates)
	fit = optim.pml(fit, model = submodel, control = pml.control(trace=0))
	ancMl = ancestral.pml(fit, "ml")

	# Get human and ancestral nodes
	human = giveAnc(ancMl, node = extant_species)

	hcAnc = giveAnc(ancMl, node = ancestral_nodes[1])
	hcgoAnc = giveAnc(ancMl, node = ancestral_nodes[2])
	gapeAnc = giveAnc(ancMl, node = ancestral_nodes[3])
	apeAnc = giveAnc(ancMl, node = ancestral_nodes[4])
	allAnc = giveAnc(ancMl, node = ancestral_nodes[5])
