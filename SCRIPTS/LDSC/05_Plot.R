...
for(i in 1:length(nodes)){
	...
	fls = list.files(path = paste0('LDSC/TOP20K_EXPAND_50kb/ADULT_', nodes[i]), pattern = '.*M', full.names = T)
...
ggbarplot(toplot, y = 'totsnps', x = 'node', color = 'node', fill = 'node', ylim = c(0,5e+6)) +
