...
dir.create('LDSC', showWarnings=F)
dir.create(paste0('LDSC/', type), showWarnings=F)
...
for(i in 1:length(nodes2)){
	...
	# Write bed
	dir.create(paste0('LDSC/', type, '/ADULT_', nodes[i]), showWarnings=F)
	write.table(tmp_bed, col.names = F, row.names = F, sep = '\t', quote = F, paste0('LDSC/', type, '/ADULT_', nodes[i], '/peakset.bed'))
}
